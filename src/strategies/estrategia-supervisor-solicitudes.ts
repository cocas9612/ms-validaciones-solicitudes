...
export class EstrategiaSupervisorSolicitudesStrategy implements AuthenticationStrategy {
  ...
  async authenticate(request: Request): Promise<UserProfile | undefined> {
    ...
    if (token) {
      ...
      let url_token = `${Keys.url_validar_token}?${Keys.arg_token}=${token}&${Keys.arg_rol_token}=${Keys.rol_supervisor_solicitudes}`;
      let r = "";
